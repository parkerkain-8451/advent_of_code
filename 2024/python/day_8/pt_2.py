...
for curr_symbol in coords:
    for i, curr_coord_1 in enumerate(coords[curr_symbol]):
        for j, curr_coord_2 in enumerate(coords[curr_symbol][i + 1 :]):
            ...
            mult = 0
            candidate = curr_coord_1[0] - (delta_x * mult), curr_coord_1[1] - (
                delta_y * mult
            )
            while (
                candidate[0] >= 0
                and candidate[0] < len(input)
                and candidate[1] >= 0
                and candidate[1] < len(input[0])
            ):
                # print(candidate)
            ...
            mult = 0
            candidate = curr_coord_2[0] + (delta_x * mult), curr_coord_2[1] - (
                delta_y * mult
            )
            while (
                candidate[0] >= 0
