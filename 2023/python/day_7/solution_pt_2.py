...
def get_hand_rank(hand):
    ...
    # Now, take the Jokers and add them to the current max
    if 'J' in counts_dict.keys() and hand != "JJJJJ":
        n_jokers = counts_dict["J"]
        del counts_dict["J"]
        curr_winner = max(counts_dict, key=counts_dict.get)
        counts_dict[curr_winner] += n_jokers
...
print(f"Part two answer: {sum(result_list)}")
