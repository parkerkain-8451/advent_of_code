...
if __name__ == "__main__":
    ...
    for curr_line in lines:
        ...
        if curr_line.startswith("seeds:"):
            ...
            while len(pieces) > 0:
                start = pieces.pop(0)
                length = pieces.pop(0)
                seeds += [x for x in range(start, start + length)]
                print(len(seeds))
        else:
            if len(curr_line) == 0:
                continue
    ...
    # This will take maybe 10 minutes
    ...
    #     locations.append(solve_for_seed(curr_seed, lookups))
    with Pool(10) as p:
        locations = p.map(solve_for_seed, seeds)
    # locations = p.map(solve_for_seed, seeds)
